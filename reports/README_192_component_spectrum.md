...
3. Вес делится на число позиций с данным референсным тринуклеотидом. Одинаково
   частые варианты в редком и частом контексте получают разные веса.
...
5. Все 192 столбца показаны на одной линейной шкале. После появления пика
   высотой 20–27% каналы с частотой \(10^{-4}\) визуально кажутся нулевыми.
...
Используются 12 направленных замен относительно референсной цепи:
...
- \(O_{\operatorname{context}(v)}\) — число позиций с тем же референсным
  тринуклеотидом в анализируемой mtDNA.
...
Затем 192 значения нормируются внутри группы:
...
Именно \(F_{g,k}\) изображено на графике. После последней нормировки общий
знаменатель 252 417 сокращается. График показывает относительную композицию,
но не абсолютное количество population signal.

По 64 геномным референсным тринуклеотидам \(O\) варьирует от 54 до 624,
то есть более чем в 11 раз. Внутри небольших функциональных классов диапазон
...
|---|---:|---:|---:|---:|
| Expanded neutral-like | 21 915 | 10 948 | 192 | 191 |
| Out-of-neutral-domain | 27 643 | 3 465 | 192 | 188 |

Следовательно, впечатление, что обнаружено только несколько каналов,
неверно. В expanded neutral-like группе не поддержан raw counts только канал
`A[G>C]G`. В out-of-neutral-domain группе не поддержаны:
...
|---|---:|---:|
| Доля top-1 точного варианта | 4,76% | 26,78% |
| Доля top-10 точных вариантов | 23,5% | 96,5% |
| Доля top-10 192-каналов | 40,7% | 97,3% |
...
В более широком подсчёте всего 14 haplogroup-defining вариантов дают 70,35%
контекстно-нормированного out-of-neutral-domain веса.
...
Нормировка также скрывает различие абсолютного signal:

| Группа | Raw carrier counts | Raw opportunity-adjusted weight до sum-to-one |
|---|---:|---:|
| Expanded neutral-like | 5 240 653 | 0,085534 |
| Out-of-neutral-domain | 1 607 274 | 0,022234 |

Out-of-neutral-domain содержит только 30,7% от neutral-like raw count total,
а его opportunity-adjusted signal составляет около 26,0%. Но после
раздельной нормировки обе группы изображаются как спектры с суммой 1. Эти
totals являются суммой carrier calls по вариантам и могут превышать число
людей, поскольку один человек несёт несколько mtDNA-вариантов.
...
| Точный вариант | 192-канал | Combined carrier count | Genome context opportunities | Count / opportunity | Доля всего спектра |
|---|---|---:|---:|---:|---:|
| `m.16519T>C` | `G[T>C]C` | 162 033 | 106 | 1 528,6 | 26,78% |
| `m.750A>G` | `G[A>G]A` | 248 424 | 201 | 1 235,9 | 21,66% |
| `m.15326A>G` | `A[A>G]C` | 250 029 | 495 | 505,1 | 8,85% |

У `m.750A>G` и `m.15326A>G` почти одинаковое число носителей. Однако
контекст `GAA` встречается 201 раз, а `AAC` — 495 раз. После opportunity
normalization `m.750A>G` получает примерно в 2,45 раза больший вес.

`m.16519T>C` имеет меньше носителей, но контекст `GTC` встречается только 106
раз. Поэтому этот вариант становится самым высоким пиком общего спектра.
...
|---|---:|---:|---:|---|---:|
| `G[T>C]C` | 26,83% | 59 | 40 | `m.16519T>C` | 99,82% |
| `G[A>G]A` | 21,67% | 118 | 29 | `m.750A>G` | 99,94% |
| `T[A>G]A` | 17,84% | 119 | 36 | `m.1438A>G` | 57,80% |
| `C[A>G]C` | 9,84% | 202 | 64 | `m.8860A>G` | 97,90% |
| `A[A>G]C` | 8,86% | 183 | 97 | `m.15326A>G` | 99,84% |

Например, в канале `G[T>C]C` наблюдаются 40 разных точных SNV. Но
`m.16519T>C` даёт 99,82% веса этого канала. График показывает один высокий
столбец не потому, что остальные 39 вариантов отсутствуют, а потому что их
...
|---|---:|---:|---:|---:|---:|---:|
| Белок-кодирующие гены | 19 708 | 2 153 | 192 | 171 | 29,1% | 92,0% |
| РНК-кодирующие гены | 7 497 | 1 196 | 192 | 160 | 51,7% | 97,6% |
...
- 106 раз по всей анализируемой mtDNA;
- только 10 раз внутри контрольного региона.

Class-specific деление на 10 вместо 106 усиливает этот вариант примерно в
10,6 раза относительно genome-wide opportunity normalization. В результате
он формирует 96,8% control-region out-of-neutral-domain 192-спектра.
...
- направление задаётся относительно rCRS, а не ancestral allele;
...
8. абсолютный opportunity-adjusted signal до нормировки к единице.
